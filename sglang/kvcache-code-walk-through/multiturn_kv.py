...
def main():
    ...
    history_ids = []
    for i, instr in enumerate(steps, start=1):
        prompt = f"{instr}: {essay}"
        new_ids = tok.encode(prompt, add_special_tokens=False)
        all_ids = history_ids + new_ids

        cache.insert(all_ids)
        print(f"\nTurn {i}, instruction inserted.")

        inputs = torch.tensor([all_ids], device=device)
        cfg = GenerationConfig(max_new_tokens=300, do_sample=True,
        ...
        print(f"Model wrote essay version: {essay[:60]}…")

        cache.insert(resp_ids)
        print("Edited-essay inserted.")
